class PolicyMonitor {
    ...
    hidden [void]ConfigureLogAnalytics() {
        # Placeholder for wiring the data collector API; keep non-blocking for tests/offline runs
        if (-not $this.WorkspaceId -or -not $this.LogAnalyticsKey) {
            Write-Verbose "Log Analytics configuration skipped: workspace or key not provided"
        }
    }
